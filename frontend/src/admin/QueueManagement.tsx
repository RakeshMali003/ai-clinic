            ...
            <div className="bg-white rounded-xl border border-gray-200">
                ...
                <div className="divide-y divide-gray-200">
                    {queue.filter(p => p.status === 'waiting').map(patient => (
                        <div
                            key={patient.id}
                            className={`p-6 hover:bg-gray-50 transition-colors ${patient.priority === 'urgent' ? 'bg-red-50' : ''
                                }`}
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`rounded-lg p-4 min-w-[80px] text-center ${patient.priority === 'urgent'
                                            ? 'bg-red-100 border-2 border-red-500'
                                            : 'bg-blue-100 border border-blue-300'
                                        }`}>
                                        <p className="text-xs font-medium text-gray-600 mb-1">Token</p>
                                        <p className={`text-3xl font-bold ${patient.priority === 'urgent' ? 'text-red-700' : 'text-blue-700'
                                            }`}>
                                            {patient.tokenNumber}
                                        </p>
                                    </div>
